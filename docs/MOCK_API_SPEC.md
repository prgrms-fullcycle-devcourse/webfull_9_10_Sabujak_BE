...
- `GET /capsules/{slug}/message-count/stream` 와 `DELETE /capsules/{slug}/messages/{messageId}` 는 실제 서버 구현 기준 endpoint이며, 현재 mock 응답은 아직 지원하지 않습니다.
...
| Message | 메시지 삭제 (planned only)           | `DELETE` | `/capsules/{slug}/messages/{messageId}` |
